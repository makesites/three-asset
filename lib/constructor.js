...
THREE.Asset.prototype = {
	...
	load: function ( source, callback ) {
		...
		if( !source.length ) return this._callback( lod );
		for(var i in source ){
			var file = source[i];
			this.loadSource(file, function( geometry, materials ){
				var object = self.getObject( geometry, materials );
				lod.addLevel( object, loaded*distance);
				loaded++;
				// exit if all done
